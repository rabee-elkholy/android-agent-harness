...
2. **Safe Release Creation (`--notes-file`)**: Always write release notes to a clean UTF-8 markdown file and publish using `gh release create <tag> --verify-tag --notes-file <file>`. Never pass multiline markdown as command-line strings to avoid PowerShell/Bash escape character corruption. Run the complete selftest suite and release validation, push the release commit, and require successful CI for that exact commit before tagging. Wait for successful tag validation before publishing; PyPI uploads independently recheck both workflows. Local-only `--no-push` preparation creates a commit without a tag, and `--skip-tests` is restricted to dry runs.
