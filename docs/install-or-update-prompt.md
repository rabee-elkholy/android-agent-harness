...
`<kit-dir>` is `<cache-root>\kit` or `<cache-root>/kit` at detached `v1.1.1`.
...
`git clone --depth 1 --branch v1.1.1 --single-branch https://github.com/rabee-elkholy/android-agent-harness.git <staging-dir>`
