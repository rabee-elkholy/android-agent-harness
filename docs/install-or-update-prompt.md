...
> **Kit version**: `v1.0.38` — **SHA-256**: `38f29580d06e00cda2209b2ed9a8804a9a8ca4e88fff1b3a64af96f43e018f46` (SHA-256 of every byte after this line; verify first — mismatch = STOP)
...
3. `<kit-dir>` is `%USERPROFILE%\.android-harness\kit` (Windows) or `~/.android-harness/kit`. Reuse only at detached `v1.0.38` with matching version and checksum `files`.

## Phase 2: Kit bootstrap approval
If cache is invalid, show staging, kit, rollback paths and commands:
```text
git clone --depth 1 --branch v1.0.38 --single-branch https://github.com/rabee-elkholy/android-agent-harness.git <staging-dir>
