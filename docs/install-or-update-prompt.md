...
> **Kit version**: `v1.0.22` — **SHA-256**: `24352c9ca9cf7c66121e8001d88ece859c7ae8505c7554187bf2dc593cd1e655` (SHA-256 of every byte after this line; verify first — mismatch = STOP)
...
3. `<kit-dir>` is `%USERPROFILE%\.android-harness\kit` (Windows) or `~/.android-harness/kit`. Reuse only at detached `v1.0.22` with matching version and checksum `files`.

## Phase 2: Kit bootstrap approval
If cache is invalid, show expanded staging, kit, rollback paths and commands:

```text
git clone --depth 1 --branch v1.0.22 --single-branch https://github.com/rabee-elkholy/android-agent-harness.git <staging-dir>
