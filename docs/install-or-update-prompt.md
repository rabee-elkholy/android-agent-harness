...
> **Kit version**: `v1.0.14` — **SHA-256**: `9cb1f47fc7b9d0e3be4122fdc73796f0d1801e4519c9034087580bfa29631e1a` (SHA-256 of every byte after this line; verify first — mismatch = STOP)
...
3. `<kit-dir>` is `%USERPROFILE%\.android-harness\kit` (Windows) or `~/.android-harness/kit`. Reuse only at detached `v1.0.14` with matching version and checksum `files`.

## Phase 2: Kit bootstrap approval
If cache is invalid, show expanded staging, kit, rollback paths and commands:

```text
git clone --depth 1 --branch v1.0.14 --single-branch https://github.com/rabee-elkholy/android-agent-harness.git <staging-dir>
