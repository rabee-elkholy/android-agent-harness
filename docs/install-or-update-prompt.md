...
> **Kit version**: `v1.0.34` — **SHA-256**: `ccea77529a7718d08b7a43a96393c30c0401542aaca7166ddca8d400df5edd9c` (SHA-256 of every byte after this line; verify first — mismatch = STOP)
...
3. `<kit-dir>` is `%USERPROFILE%\.android-harness\kit` (Windows) or `~/.android-harness/kit`. Reuse only at detached `v1.0.34` with matching version and checksum `files`.

## Phase 2: Kit bootstrap approval
If cache is invalid, show staging, kit, rollback paths and commands:
```text
git clone --depth 1 --branch v1.0.34 --single-branch https://github.com/rabee-elkholy/android-agent-harness.git <staging-dir>
...
Show discovered DI, ViewModel base, Room, UI, capabilities, legacy overrides.
