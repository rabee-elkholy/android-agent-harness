...
> **Kit version**: `v1.0.9` — **SHA-256**: `68232db6ab722a8c7a3d0e274dd7bd4daa8d4406ffa18b0c273af24ae523d583` (SHA-256 of every byte after this line; verify first — mismatch = STOP)
...
3. `<kit-dir>` is `%USERPROFILE%\.android-harness\kit` (Windows) or `~/.android-harness/kit`. Reuse only at detached `v1.0.9` with matching version and checksum `files`.

## Phase 2: Kit bootstrap approval
If cache is invalid, show expanded staging, kit, rollback paths and commands:

```text
git clone --depth 1 --branch v1.0.9 --single-branch https://github.com/rabee-elkholy/android-agent-harness.git <staging-dir>
