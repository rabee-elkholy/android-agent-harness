...
def message_for(used_reviews: int, pending: bool, update_directive: str = "") -> str:
    ...
    return (
        ...
        "REACTIVE WAKEUP & SILENT REVIEW WAIT: After invoke_subagent, stop calling tools immediately. NEVER use schedule or timers to wait for subagents. When woken up while some reviewers are still running, OUTPUT ZERO CHAT TEXT, call no tools, and end turn silently. Only output the review table once ALL verdicts have arrived. "
        ...
        "DEVICE VERIFICATION: Install via python .agents/scripts/run_device.py install-start, and execute python .agents/scripts/run_e2e_smoke.py when DEVICE_VERIFICATION_MODE is autonomous_e2e. Present E2E evidence + manual checklist in Phase Milestone Card before ask_question. One manual phase at a time via ask_question. "
