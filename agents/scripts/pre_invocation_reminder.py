...
def message_for(used_reviews: int, pending: bool, update_directive: str = "") -> str:
    ...
    return (
        ...
        "ZERO-NOISE CHAT & LANGUAGE PARITY: Match the developer's active conversation language (Arabic/English) across all cards (Review Round, Phase Milestone, Final Delivery) and interactive modals. Never emit mechanical status updates in chat prose (e.g. 'Running tests...', 'Cleaning kapt cache...', 'Waiting for...'). Rely on IDE tool widgets for routine actions. Chat is reserved exclusively for Plan Approval, Review Round Cards (on findings), Phase Milestone Cards, and Final Delivery. "
