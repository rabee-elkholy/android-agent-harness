...
1. **Brainstorm & Interactive Clarification (The Zero-Assumption Barrier)**:
   ...
   - If ANY edge case or requirement is underspecified, **proactively interview the developer via `ask_question`** with concrete choices before authoring `implementation_plan.md`. Never guess or assume business logic from your own head. Build the plan right the first time.
