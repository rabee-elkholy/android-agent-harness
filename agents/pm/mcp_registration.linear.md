# Linear via Upstream MCP Server — Registration Playbook

Linear integration is **configuration, not code**: the harness ships no Linear
client. Register Linear's official upstream MCP server, keep credentials in the
user-level config exactly like Zoho Sprints (rules section 5), and reuse the
same QA-centric handoff policy.

## 1. Prerequisites

- A Linear workspace with API access.
- The official Linear MCP server (see Linear's documentation for the current
  remote URL or `mcp-server-linear` package) and an OAuth or API-key flow
  approved by your organization.
- Never paste tokens into this repository. User-level files only:
  `~/.android-harness/linear.json` (same pattern as
...
```json
{
  "mcpServers": {
    "linear": {
      ...
      "env": { "LINEAR_CONFIG": "~/.android-harness/linear.json" }
    }
  }
}
```

Follow current official Linear MCP documentation for the exact command,
...
Kit canonical status is set by `agents/scripts/pm_policy.py`. Linear labels:

| Kit canonical | Linear label | Notes |
|---|---|---|
| `in_progress` | `In Progress` | Set when work starts |
| `ready_to_retest` | `In Review` | Set after all device phases Pass |

Denied Linear statuses (never set, never declared in a handoff): `Done`,
`Canceled`, `Cancelled`.

## 4. Mutation trigger phrase

Mutations happen only on the explicit chat phrase **`update linear`** (the
...
- Validate offline before posting: `validate_handoff(text, lang_mode, "linear")`
  from `agents/scripts/pm_policy.py`.

## 6. Credential isolation checklist

- Secrets live only in `~/.android-harness/linear.json`.
