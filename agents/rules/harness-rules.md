...
- **Dual Device Verification Modes**:
  - **Mode A: `autonomous_e2e` (Autonomous E2E Verification - Default)**:
    ...
    2. **MANDATORY**: Run `python .agents/scripts/run_e2e_smoke.py` (with `--auto-diff` or `--flow <path/to/flow.yaml>`). For feature tasks, new screens, or UI modifications, the agent **MUST act as a true Senior QA Tester** by generating a declarative flow in `.agents/e2e_flows/<feature>.yaml` or exercising the modified activities/screens directly with explicit interactions (`tapOn`, `inputText`, `scrollUntilVisible`, `assertVisible`), capturing screenshots for every major step, rather than merely verifying launcher launch.
